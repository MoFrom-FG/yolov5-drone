/***************************************************************************************************************************
 * autonomous_landing_on_ground.cpp
 *
 * Author: Eason Yi
 *
 ...
 *
 * 说明: 精准降落之降落在地上
 *      1. 订阅目标位置(来自prometheus_detection视觉功能包的二维码降落板ROS节点)
 *      2. 降落算法及降落策略
 *      3. 发布上层控制指令到px4_sender (prometheus_msgs::ControlCommand)
***************************************************************************************************************************/
//ROS 头文件
#include <ros/ros.h>
#include <iostream>
...
#define LANDPAD_HEIGHT 0.0 //降落板在地面站，高度为0
#define NODE_NAME "autonomous_landing_on_ground"
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>全 局 变 量<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
//---------------------------------------Drone---------------------------------------------
prometheus_msgs::DroneState _DroneState;    //飞机当前状态，详细定义可参考 DroneState.msg
nav_msgs::Odometry GroundTruth; //降落板的空间位置（在仿真中可用，配合调试模式使用）
Eigen::Matrix3f R_Body_to_ENU;  //机体系到惯性系转换函数
std_msgs::Bool flag_start;  //任务开始标志位
//---------------------------------------Vision---------------------------------------------
Detection_result landpad_det;   //降落板的检测结果
Eigen::Vector3f pos_des_prev;   //期望的飞机相对于降落板的空间位置

float kpx_land,kpy_land,kpz_land;                                                 //控制参数 - 比例参数
float start_point_x,start_point_y,start_point_z;    //期望飞机的空间位置
int debug_mode; //调试模式标志（用于仿真调试）
double roll, pitch, yaw;//定义存储r\p\y的容器
bool use_ukf;   
bool moving_target; //是否为移动目标：否
Eigen::VectorXd state_fusion;
Eigen::Vector3f camera_offset;  //相机安装偏移量
//---------------------------------------Track---------------------------------------------
float distance_to_setpoint; //飞机中心距离降落板的距离
float distance_thres;   //距离阈值
//---------------------------------------Output---------------------------------------------
...
{
    ros::init(argc, argv, "autonomous_landing_on_ground");
    ros::NodeHandle nh("~");
